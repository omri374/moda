...
def anomaly_detect_ts(x, max_anoms=0.1, direction="pos", alpha=0.05, only_last=None, period = None,
                      threshold=None, e_value=False, longterm=False, piecewise_median_period_weeks=2,
    ...
    assert x.values.dtype in [np.int64, int, float], 'Values of the series must be number'
    ...
    for series in all_data:
        if series.size < period* 2:
            print('Anomaly detection needs at least 2 periods worth of data')
            #continue
